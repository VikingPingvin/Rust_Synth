...
impl Synthesizer {
    fn new() -> Synthesizer {
        Synthesizer {sample_rate: 44100, channel_count: 2, curr_freq: 440.0, curr_sample: 0, volume: 0.25}
    }

    fn samples_iter(&mut self) -> SamplesIter {
        SamplesIter {synth: self}
    }
    ...
    fn next_sampe(&mut self) -> f32 {
        ...
        self.curr_sample += 1;
        return self.volume * ((PI * 2.0 * curr_sec) as f32 * self.curr_freq).sin()
    }
}
...
fn playbeep(){
    ...
    let mut synth_arc_mtx = Arc::new(Mutex::new(Synthesizer::new()));
    {
